...
\section{工作经历}
  \resumeSubHeadingListStart
    ...
    \resumeSubheading
      {Mercury Insurance}{上海}
      {资深软件工程师}{2025年1月至今}
      \resumeItemListStart
        \resumeItem{基于跨部门的用户需求设计开发机器学习平台。}
        \resumeItem{优化部署于 \emph{Sagemaker} 上的机器学习模型。}
        \resumeItem{优化机器学习模型生命全周期，缩短包含模型开发，特征加工和模型发布的全流程，加快业务迭代速度。}
      \resumeItemListEnd

    \resumeSubheading
      {Adevinta (ebay classifieds group)}{上海}
      {资深软件工程师(L26)}{2020年11月 -- 2024年12月}
      \resumeItemListStart
      \resumeItem{设计开发基于Airflow和Spark的数据平台，使用云原生技术运行于\emph{EKS}。}
      \resumeItem{开发维护内部的Spark版本，基于开源社区项目，添加对\emph{AWS} glue, delta.io, Arm架构和云原生的支持。}
      \resumeItem{基于开源Jupyter和VSCode开发内部的Notebook工具，用于数据分析与机器学习开发。前端基于\emph{React}，后端基于\emph{go}。}
      \resumeItem{定期复查与优化\emph{AWS}资源使用成本，保证资源能够充分合理地使用。}
       \resumeItemListEnd
...
%-------------------------------------------
    ...
    \resumeSubheading
      ...
      \resumeItemListStart
        ...
        \resumeItem{开发内部自助工具以帮助销售与客服更方便地获取账务数据。}
        \resumeItem{自动化部分账户流程。}
      \resumeItemListEnd
...
\section{项目}
    \resumeSubHeadingListStart
      \resumeProjectHeading
          {\textbf{机器学习平台} $|$ \emph{AWS(EKS), kubernetes, feast, mlflow, kubeflow, kserving}}{2025年1月至今}
          \resumeItemListStart
          \resumeItem{在 \emph{EKS} 上部署 \emph{feast} 管理机器学习指标。提供批量离线与在线指标服务。}
          \resumeItem{在 \emph{EKS} 上部署 \emph{MLFlow} 管理模型实验与 artifact，版本。}
          \resumeItemListEnd
...
\section{技术栈}
 \begin{itemize}[leftmargin=0.15in, label={}]
    \small{\item{
     ...
     \textbf{框架}{: Spark, Hadoop, Flask, pandas, Matplotlib, Airflow, Argoflow, React, Beego, Springboot, Feast, MLFlow } \\
